...
async function transpileTypeScript(code: string, fileName: string) {
  const transformResult = await transform(fileName, code, {
    typescript: {
      onlyRemoveTypeImports: true,
    },
  })

  const formatResult = await format(fileName, transformResult.code, formatConfig)

  return formatResult.code
}

async function transpileVueTypeScript(code: string, fileName: string) {
  let changed = false

  const output = await replaceAsync(
    code,
    /<script\b([^>]*?)\blang=(['"])ts\2([^>]*)>([\s\S]*?)<\/script>/g,
    async (fullMatch, beforeLang: string, quote: string, afterLang: string, script: string) => {
      const transpiled = await transpileTypeScript(script, `${fileName}.ts`)
...
function resolveSnippetPath(targetPath: string, aliasRoots: SnippetAliasRoot[]) {
...
async function renderSnippet(target: SnippetTarget, sourcePath: string) {
  ...
  if (ext === '.ts') {
    const transpiled = await transpileTypeScript(snippet, sourcePath)
  ...
  if (ext === '.vue' && /<script\b[^>]*lang=['"]ts['"][^>]*>/.test(snippet)) {
    const transpiled = await transpileVueTypeScript(snippet, sourcePath)

    if (transpiled) {
      return [
        '::: code-group',
        createFence('vue', snippet, target.title || 'vue ts'),
        createFence('vue', transpiled, 'vue js'),
        ':::',
      ].join('\n')
    }
  }
...
async function transformMarkdown(code: string, aliasRoots: SnippetAliasRoot[]) {
  ...
  for (const line of lines) {
    ...
    const rendered = (await renderSnippet(target, sourcePath))
      .split('\n')
...
export function vitepressSnippetPlugin(options: SnippetPluginOptions) {
  return {
    ...
    async transform(code: string, id: string) {
      ...
      return await transformMarkdown(code, options.aliasRoots)
    },
