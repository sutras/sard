...
import packageJson from 'sard/package.json' with { type: 'json' }
...
const snippetAliasRoots = [
  {
    find: '@demo',
    replacement: path.join(rootDir, 'src', 'views', 'components'),
  },
  {
    find: '@comp',
    replacement: path.join(rootDir, 'packages', 'sard', 'components'),
  },
  {
    find: '@lib',
    replacement: path.join(rootDir, 'packages', 'sard'),
  },
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (key === 'group') {
      ...
      while (index + 1 < lines.length && lines[index + 1].startsWith('  ')) {
        index += 1
        const nestedPair = lines[index].trim().match(/^([A-Za-z][\w-]*):\s*(.*)$/)
...
function buildSidebar(
  ...
  const groups = new Map<string, { order: number; items: DefaultTheme.SidebarItem[] }>()
...
export default defineConfig({
  ...
  themeConfig: {
    ...
    nav: [
      ...
      { text: '工具', link: '/utilities/date' },
      {
        text: packageJson.version,
        items: [
          {
            text: '更新日志 (Github)',
            link: 'https://github.com/sutras/sard/blob/main/CHANGELOG.md',
          },
          {
            text: '更新日志 (Gitee)',
            link: 'https://gitee.com/sutras/sard/blob/main/CHANGELOG.md',
          },
        ],
      },
      {
        ...
        items: [
          ...
          {
            text: 'Sard Uniapp',
            link: 'https://sard-uniapp.wzt.zone/',
          },
        ],
      },
    ],
    ...
    socialLinks: [
      { icon: 'github', link: 'https://github.com/sutras/sard' },
      { icon: 'gitee', link: 'https://gitee.com/sutras/sard' },
    ],
    footer: {
    ...
    editLink: {
      pattern: 'https://github.com/sutras/sard/edit/main/packages/docs/:path',
