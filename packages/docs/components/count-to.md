...
<<< @demo/count-to/demo/Basic.vue

### 小数位数

使用 `precision` 属性设置小数位数。

<<< @demo/count-to/demo/Precision.vue

### 分隔符

使用 `separator` 属性设置千分位分隔符。

<<< @demo/count-to/demo/Separator.vue

### 持续时间

使用 `duration` 属性设置滚动时长。

<<< @demo/count-to/demo/Duration.vue

### 随机数字

`value` 属性改变时会自动滚动到最新的值。

<<< @demo/count-to/demo/Random.vue
