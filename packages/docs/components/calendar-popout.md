...
<<< @demo/calendar-popout/demo/Basic.vue

### 类型

日历组件可以选择单个值、多个值以及进行范围选择。

<<< @demo/calendar-popout/demo/Type.vue

### 自定义日期范围

可以使用 `min` 和 `max` 属性限制可以选择的日期的范围。

<<< @demo/calendar-popout/demo/MinMax.vue
...
<<< @demo/calendar-popout/demo/MaxDays.vue

### 禁用日期

`disabledDate` 属性配置的函数接收一个日期对象，如果此函数返回真则禁用这个日期。

<<< @demo/calendar-popout/demo/DisabledDate.vue
...
<<< @demo/calendar-popout/demo/WeekStartsOn.vue

### 格式化日期

`formatter` 属性可以配置一个接收 `CalendarDay` 类型的对象，通过此对象可以自定义当前日期展示的内容和样式。

<<< @demo/calendar-popout/demo/Formatter.vue
...
<<< @demo/calendar-popout/demo/Several.vue

## API

### CalendarPopoutProps

继承 [`CalendarProps`](./calendar#CalendarProps) 和 [`FormPopoutProps`](./popout#FormPopoutProps)。
