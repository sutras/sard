...
<<< @demo/dropdown/demo/Basic.vue

### 占位符

占位符会在未选中值时显示说明文字。

<<< @demo/dropdown/demo/Placeholder.vue

### 添加 label

相较于占位符， `label` 会一直显示。

<<< @demo/dropdown/demo/Label.vue

### 向上展开

底部空间不足时可以配置向上弹出菜单。

<<< @demo/dropdown/demo/Direction.vue

### 禁用

禁用的菜单项不可点击。

<<< @demo/dropdown/demo/Disabled.vue

### 自定义内容

除了显示菜单项，下拉菜单还可以显示任意内容，这时可以使用 `v-model:visible` 控制显隐。

<<< @demo/dropdown/demo/Content.vue
...
<<< @demo/dropdown/demo/Async.vue

### 可取消选择

如果想点击时取消选中的选项，可以使用 `togglable` 属性。

<<< @demo/dropdown/demo/Togglable.vue

### 分隔线

可以使用 `separator` 来设置菜单与面板之间的分割线类型。

<<< @demo/dropdown/demo/Separator.vue
...
type DropdownCloseType = 'overlay' | 'away' | 'other-button' | 'option' | 'button'
```
