...
<<< @demo/steps/demo/Basic.vue

### 居中

居中每个步骤的图标和文案。

<<< @demo/steps/demo/Center.vue

### 文字在上

可以使用 `reverse` 属性将水平排列时的文字和图标位置调换。

<<< @demo/steps/demo/Reverse.vue

### 垂直步骤条

设置 `direction="vertical"` 可以垂直排列。

<<< @demo/steps/demo/Vertical.vue

### 垂直居中

将图标和文案垂直居中。

<<< @demo/steps/demo/VerticalCenter.vue

### 自定义图标

使用 `Step` 组件 `icon` 插槽自定义图标。

<<< @demo/steps/demo/Icon.vue

### 自定义颜色

使用 `css` 变量设置元素样式。

<<< @demo/steps/demo/Color.vue

### 当前步骤状态

可以设置当前处理中的步骤的状态为 `finish` 来模拟只有“未处理”和“已处理”两个状态的步骤条。

<<< @demo/steps/demo/Status.vue

### 错误步骤

可以设置当前处理中的步骤的状态为 `error` 表示步骤运行错误。

<<< @demo/steps/demo/ErrorStatus.vue

### 自定义各步骤状态

每个步骤都可以使用 `status` 属性设置其状态。

<<< @demo/steps/demo/StepStatus.vue

### 默认插槽

使用 `Step` 组件实现自定义内容，必须传递 `index` 属性。

<<< @demo/steps/demo/Slot.vue
