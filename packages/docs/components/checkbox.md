...
<<< @demo/checkbox/demo/Basic.vue

### 只读和禁用

只读或禁用后不可点击。

<<< @demo/checkbox/demo/DisabledReadOnly.vue

### 图标大小

使用 `size` 属性设置图标大小。

<<< @demo/checkbox/demo/Size.vue

### 图标颜色

使用 `checked-color` 属性设置选中时的图标颜色。

<<< @demo/checkbox/demo/Color.vue

### 图标类型

设置 `type` 属性为 `circle` 可以使图标变为圆形。

<<< @demo/checkbox/demo/Type.vue
...
<<< @demo/checkbox/demo/Icon.vue

### 复选框组

复选框组用于收集选中状态的复选框的值。

<<< @demo/checkbox/demo/Group.vue

### 排列方向

将 `direction` 属性设置为 `horizontal` 后，复选框组会变成水平排列。

<<< @demo/checkbox/demo/Direction.vue

### 自动渲染复选框

使用 `options` 属性设置可选项。

<<< @demo/checkbox/demo/GroupOptions.vue
...
<<< @demo/checkbox/demo/Custom1.vue

结合 `list` 组件使用：

<<< @demo/checkbox/demo/Custom2.vue

复选框组里面 `checkbox` 组件，会自动判断选中状态；可以给 `checkbox` 组件添加 `readonly` 属性以便将点击操作交给其他组件。

<<< @demo/checkbox/demo/Custom3.vue

### 不确定状态

<<< @demo/checkbox/demo/Indeterminate.vue
...
| indeterminate  | 是否处于不确定状态               | boolean              | false    |
...
| 事件              | 描述                 | 类型                     |
| ----------------- | -------------------- | ------------------------ |
| update:modelValue | 复选框组值改变时触发 | `(value: any[]) => void` |
...
| CSS 变量                           | 值                             |
| ---------------------------------- | ------------------------------ |
| `--s-checkbox-group-column-gap`    | `var(--s-size-sm)`             |
| `--s-checkbox-group-row-gap`       | `var(--s-size-xs)`             |
| `--s-checkbox-icon-font-size`      | `20px`                         |
| `--s-checkbox-icon-color`          | `var(--s-text-color-fourth)`   |
| `--s-checkbox-icon-color-checked`  | `var(--s-color-primary)`       |
| `--s-checkbox-icon-color-disabled` | `var(--s-text-color-disabled)` |
| `--s-checkbox-label-margin-start`  | `var(--s-size-xs)`             |
