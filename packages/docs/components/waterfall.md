...
<<< @demo/waterfall/demo/Basic.vue

`SimulatedImage.vue`

<<< @demo/waterfall/demo/SimulatedImage.vue
...
<<< @demo/waterfall/demo/KnownSize.vue

### 真实案例

需同时监听 `image` 组件的 `load` 和 `error` 事件，确保 `onLoad` 回调函数能调用，无论图片加载成功或失败。

<<< @demo/waterfall/demo/TrueCase.vue
...
<<< @demo/waterfall/demo/MaxWait.vue

### 大图

可使用 `column-gap` 和 `row-gap` 设置瀑布流项的间隔大小，单位为 px。

<<< @demo/waterfall/demo/BigImage.vue

### 自定义列数

可使用 `columns` 属性设置任意的列数。

<<< @demo/waterfall/demo/Columns.vue
...
<<< @demo/waterfall/demo/Dynamic.vue
