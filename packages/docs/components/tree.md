...
| single-selectable     | 节点是否可被选择（单选）                                                     | boolean                                                                                              | false           |
| leaf-only             | 是否只能选择叶子节点（单选）                                                 | boolean                                                                                              | false           |
| current               | 当前选择的节点的 key（单选）                                                 | string \| number                                                                                     | -               |
...
| load                  | 加载子树数据的方法，仅当 lazy 属性为true 时生效                              | `(node?: TreeStateNode) => Promise<TreeNaturalNode[]> \| TreeNaturalNode[]`                          | -               |
| auto-height           | 默认只可展示一行文字，设置此属性可让节点高度跟随内容变化，只可用在非拖拽模式 | boolean                                                                                              | false           |
| allow-drag            | 判断节点能否被拖拽，返回 false 不允许拖拽                                    | `(node: TreeStateNode) => boolean`                                                                   | -               |
| before-drop           | 拖拽完放置前回调，返回 `false` 或 `Promise{<rejected>}` 可阻止放置           | `(draggingNode: TreeStateNode, dropNode: TreeStateNode, type: TreeDropType) => any \| Promise\<any>` | -               |
| before-edit           | 节点编辑前回调，返回 `false` 或 `Promise{<rejected>}` 可阻止编辑             | `(node: TreeStateNode, title: string, type: TreeEditType) => any \| Promise \<any>`                  | -               |

### TreeEmits

| 事件           | 描述                   | 类型                                                       |
| -------------- | ---------------------- | ---------------------------------------------------------- |
| update:current | 选择节点后触发（单选） | `(key: string \| number, node: TreeStateNode) => void`     |
| select         | 选择节点后触发（单选） | `(key: string \| number, node: TreeStateNode) => void`     |
| check          | 点击树节点复选框时触发 | `(event: {checked: boolean; node: TreeStateNode}) => void` |
| node-click     | 点击树节点时触发       | `(event: {event: any; node: TreeStateNode}) => void`       |
...
| getExpandedKeys    | 获取展开状态的节点     | `(keys: (string \| number)[]) => void`               |
...
| CSS 变量                            | 值                              |
| ----------------------------------- | ------------------------------- |
| `--s-tree-border-color`             | `var(--s-border-color)`         |
| `--s-tree-bg`                       | `var(--s-bg-color-container)`   |
| `--s-tree-loading-size`             | `var(--s-font-size)`            |
| `--s-tree-loading-color`            | `var(--s-text-color-fourth)`    |
| `--s-tree-loading-margin-y`         | `10px`                          |
| `--s-tree-node-height`              | `var(--s-content-height-sm)`    |
| `--s-tree-node-padding-x`           | `var(--s-content-padding-x)`    |
| `--s-tree-node-padding-y`           | `var(--s-content-padding-y)`    |
| `--s-tree-node-bg`                  | `var(--s-bg-color-container)`   |
| `--s-tree-node-bg-active`           | `var(--s-bg-color-active)`      |
| `--s-tree-node-box-shadow-dragging` | `var(--s-box-shadow-secondary)` |
| `--s-tree-node-loading-margin-end`  | `var(--s-size-xs)`              |
| `--s-tree-toolbar-margin-end`       | `-12px`                         |
| `--s-tree-indent-width`             | `var(--s-size-xl)`              |
| `--s-tree-arrow-padding-x`          | `var(--s-size-sm)`              |
| `--s-tree-arrow-font-size`          | `var(--s-font-size-sm)`         |
| `--s-tree-arrow-color`              | `var(--s-text-color-tertiary)`  |
| `--s-tree-selection-padding-x`      | `var(--s-size-sm)`              |
| `--s-tree-edit-padding-x`           | `var(--s-size-sm)`              |
| `--s-tree-edit-font-size`           | `18px`                          |
| `--s-tree-edit-color`               | `var(--s-text-color-fourth)`    |
| `--s-tree-edit-opacity-active`      | `var(--s-opacity-active)`       |
| `--s-tree-level-btn-padding-x`      | `var(--s-size-sm)`              |
| `--s-tree-level-btn-font-size`      | `18px`                          |
| `--s-tree-level-btn-color`          | `var(--s-text-color-fourth)`    |
| `--s-tree-level-btn-opacity-active` | `var(--s-opacity-active)`       |
| `--s-tree-drag-padding-x`           | `var(--s-size-sm)`              |
| `--s-tree-drag-font-size`           | `18px`                          |
| `--s-tree-drag-color`               | `var(--s-text-color-fourth)`    |
| `--s-tree-drag-opacity-active`      | `var(--s-opacity-active)`       |
