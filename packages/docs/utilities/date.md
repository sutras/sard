...
```ts
function getDaysBeforeFirstDay(year: number, month: number, weekStartsOn?: number): number
```
...
```ts
function getDaysAfterLastDay(year: number, month: number, weekStartsOn?: number): number
```
...
```ts
function getPrevMonthTailDays(year: number, month: number, weekStartsOn?: number): Date[]
```
...
```ts
function getNextMonthHeadDays(year: number, month: number, weekStartsOn?: number): Date[]
```
