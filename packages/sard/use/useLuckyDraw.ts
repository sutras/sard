...
export const useLuckyDraw = (options: UseLuckyDrawOptions) => {
  ...
  const getDecelStopIndex = () => {
    ...
    while (++i) {
      ...
      const currentSpeed = unref(maxSpeed) - easeOut(progress) * (unref(maxSpeed) - unref(minSpeed))
  ...
  const tick = () => {
    switch (stage) {
      ...
      case 'ACCELERATED': {
        ...
        const currentSpeed =
          unref(minSpeed) + easeIn(progress) * (unref(maxSpeed) - unref(minSpeed))
      ...
      case 'DECELERATED': {
        ...
        currentIndex = decelStartIndex + easeOut(progress) * (decelStopIndex - decelStartIndex)
  ...
  const stop = (index?: number) => {
    if (decelStopActiveIndex === undefined) {
      decelStopActiveIndex = index === undefined ? random(0, unref(count) - 1) : index
    }
  }
