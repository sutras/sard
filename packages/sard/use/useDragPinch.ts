...
export interface UseTouchOptions {
  onOffset?: (offsetX: number, offsetY: number, event: TouchEvent) => void
...
export function useDragPinch(options: UseTouchOptions) {
  ...
  const onTouchMove = (event: TouchEvent) => {
    ...
    if (prevScalePoints && currScalePoints) {
      const prevDistance = getTwoPointsDistance(...(prevScalePoints as [Point, Point]))
      const currDistance = getTwoPointsDistance(...(currScalePoints as [Point, Point]))
