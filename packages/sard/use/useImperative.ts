...
export function useImperative<T = any>(name: string, imperative: T, id: Ref<string>) {
  onMounted(() => {
    if (id.value) {
...
export function getAvailableImperative<T = any>(name: string, id: string): T | void {
  ...
  if (imperativeItems) {
    for (let i = imperativeItems.length - 1; i >= 0; i--) {
      ...
      if (imperativeItem && imperativeItem.show) {
...
export function getImperatives<T = any>(name: string, id: string): ImperativeItem<T>[] | undefined {
