...
export const defaultOptionKeys: Required<OptionKeys> = {
  label: 'label',
  value: 'value',
  disabled: 'disabled',
  children: 'children',
...
export function useOptionKeys(
  ...
  const getDisabled = (option: any) => chainGet(option, aliasProps.value.disabled)
  const getChildren = (option: any) => chainGet(option, aliasProps.value.children)
