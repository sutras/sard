...
/**
 * @description: 判断是否为非空数组
 * @param {any} target
 * @return {boolean}
 */
export function isNoEmptyArray(target: any): target is [] {
  return Array.isArray(target) && target.length > 0
}
 ...
 * @return {boolean}
 */
export function isObject(target: any): target is object & { [key: PropertyKey]: any } {
 ...
 * @return {boolean}
 */
export function isConfigObject(target: any): target is Record<PropertyKey, any> {
  return isPlainObject(target) && !isVNode(target)
}
