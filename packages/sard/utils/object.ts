 ...
 */
export function omit<T extends object, U extends keyof T>(object: T, keys: U[]) {
 ...
 */
export function pick<T extends object, U extends keyof T>(object: T, keys: U[]) {
...
/**
 * 根据指定的键数组将对象拆分为两个对象，一个包含指定键的属性，另一个包含其余属性。
 */
export function partition<T extends object, U extends keyof T>(
  object: T,
  keys: U[],
): [Pick<T, U>, Omit<T, U>] {
  const matched = {} as Pick<T, U>
  const rest = {} as Omit<T, U>
  ;(Object.keys(object) as U[]).forEach((key) => {
    if (keys.includes(key)) {
      ;(matched as any)[key] = object[key]
    } else {
      ;(rest as any)[key] = object[key]
    }
  })
  return [matched, rest]
}
 ...
 */
export function chainGet(object: any, chain?: string | number | (string | number)[]) {
  let target = object
 ...
 * @param value 要设置的值
 */
export function chainSet(object: any, chain: string | number | (string | number)[], value: any) {
  let target = object
...
export function nestedToMulti(
  ...
  { getValue, getChildren }: NestedToMultiOptionKeys,
) {
  const columns: any[] = []
  ...
  function recurse(list: any[], index = 0) {
    ...
    let selectedOption = list.find((option) => getValue(option) === selectedValue)
    if (!selectedOption) {
