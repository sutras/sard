...
function getVersion(mode: Mode, eclIndex: number, codewords: number) {
  for (let version = 1; version <= 40; version++) {
    ...
    if (rawDataCodewords >= codewords + (4 + getCharCountIndicator(mode, version)) / 8) {
      return version
    }
  }
  return 40
...
function addFinderPatterns(map: number[][], size: number) {
  for (let i = 0; i < 3; i++) {
    ...
    for (let y = 0; y < 7; y++) {
      for (let x = 0; x < 7; x++) {
        const bit =
          x === 0 || x === 6 || y === 0 || y === 6 || (x > 1 && x < 5 && y > 1 && y < 5) ? 1 : 0
...
function getLeastFiveSameColoredPenalty(getEach: (i: number, j: number) => number, size: number) {
  let score = 0
...
function getTwoByTwoSameColoredPenalty(map: number[][], size: number) {
  ...
  for (let y = 0; y < size - 2; y++) {
    for (let x = 0; x < size - 2; x++) {
      if ((map[y][x] + map[y][x + 1] + map[y + 1][x] + map[y + 1][x + 1]) % 4 === 0) {
        score += 3
      }
    }
  }
