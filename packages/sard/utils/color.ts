...
const parseUnitValue = (
  ...
  if (value.endsWith('%')) {
    ...
    return percentBase === undefined ? numberValue / 100 : (numberValue / 100) * percentBase
  }
...
export function normalizeHsla(color: Partial<HslaColor> | undefined): HslaColor {
  return {
...
export function normalizeHsva(color: Partial<HsvaColor> | undefined): HsvaColor {
  return {
    h: normalizeHue(color?.h ?? DEFAULT_COLOR.h),
    s: clamp(color?.s ?? DEFAULT_COLOR.s, 0, 100),
...
export function rgbToHsl(red: number, green: number, blue: number, alpha = 1): HslaColor {
...
function parseRgb(value: string) {
  ...
  const [r, g, b] = channels.map((item) => parseUnitValue(item, { percentBase: 255 }))
...
export function getPreviewColor(colorValue: Partial<HslaColor> | undefined, showAlpha = false) {
