...
export function getScrollIntoViewValue(
  ...
  const elementToViewportBottomOffset =
    elementOffsetTop + elementHeight - viewportScrollTop - viewportHeight + endOffset
  ...
  switch (position) {
    ...
    case 'center':
      ...
      break
    case 'end':
      nextScrollTop = elementOffsetTop + elementHeight - viewportHeight + endOffset
      break
  }
...
export function scrollToTarget(
  ...
  const { animated, vertical, ...scrollOptions } = options

  const scrollRect = scrollEl.getBoundingClientRect()
  const scrollSize = vertical ? scrollRect.height : scrollRect.width
  const scrollOffset = vertical ? scrollEl.scrollTop : scrollEl.scrollLeft
  const scrollStart = vertical ? scrollRect.top : scrollRect.left
  ...
  const offsetValue = targetStart - scrollStart + scrollOffset
  const scrollValue = getScrollIntoViewValue(
    scrollSize,
    scrollOffset,
    targetSize,
    offsetValue,
 ...
 *
 * @param {array} rects DOMRect 类型数组
 ...
 */
export function getTransformOrigin(rect: Rect, scaleRect: Rect): [number, number] {
  const ratio = scaleRect.width / rect.width
  const originX =
    (rect.x + rect.width / 2 - scaleRect.x - scaleRect.width / 2) / (ratio - 1) + rect.width / 2
  const originY =
    (rect.y + rect.height / 2 - scaleRect.y - scaleRect.height / 2) / (ratio - 1) + rect.height / 2
...
export function isRectEqual(a: DOMRect, b: DOMRect) {
