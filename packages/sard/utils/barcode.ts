...
function encodeEANDigits(data: string, structure: string, separator = '') {
  return data
    .split('')
    .map((digit, index) => {
      const digits = EAN_DIGIT_ENCODINGS[structure[index] as keyof typeof EAN_DIGIT_ENCODINGS]
...
function encodeCODE128CodeBChar(char: string) {
  ...
  assert(code >= 32 && code <= 127, `Invalid barcode input for format CODE128: ${char}`)
  return code - 32
}

function encodeCODE128(value: string, options: BarcodeRenderOptions): RawBarcodeEncoding {
  assert(value.length > 0, 'Invalid barcode input for format CODE128: empty value')
  assert(/^[\x20-\x7F]+$/.test(value), `Invalid barcode input for format CODE128: ${value}`)

  const startDigitRun = countLeadingDigits(value, 0)
  let activeSet: 'B' | 'C' = startDigitRun >= 4 && startDigitRun % 2 === 0 ? 'C' : 'B'
  ...
  const data = [...codes, checksumCode, CODE128_STOP].map((code) => CODE128_PATTERNS[code]).join('')

  return createEncoding(data, getDisplayText(value, options))
}

function encodeEAN13(value: string, options: BarcodeRenderOptions): RawBarcodeEncoding[] {
  const normalizedValue = /^[0-9]{12}$/.test(value) ? value + checksumEAN13(value) : value
...
function encodeUPC(value: string, options: BarcodeRenderOptions): RawBarcodeEncoding[] {
  const normalizedValue = /^[0-9]{11}$/.test(value) ? value + checksumUPC(value) : value
  ...
  return [
    ...
    createEncoding('101' + encodeEANDigits(normalizedValue.slice(0, 1), 'L'), '', {
      height: guardHeight,
    }),
    createEncoding(encodeEANDigits(normalizedValue.slice(1, 6), 'LLLLL'), displayText.slice(1, 6), {
      fontSize,
    }),
    createEncoding('01010', '', { height: guardHeight }),
    createEncoding(
    ...
    createEncoding(encodeEANDigits(normalizedValue.slice(11), 'R') + '101', '', {
      height: guardHeight,
    }),
...
function encodeITF14Data(value: string, options: BarcodeRenderOptions): RawBarcodeEncoding {
  assert(/^([0-9]{2})+$/.test(value), `Invalid barcode input for format ITF14: ${value}`)

  return createEncoding(
    '1010' + (value.match(/.{2}/g) ?? []).map((pair) => encodeITFPair(pair)).join('') + '11101',
    getDisplayText(value, options),
  )
}

function encodeITF14(value: string, options: BarcodeRenderOptions): RawBarcodeEncoding {
  const normalizedValue = /^[0-9]{13}$/.test(value) ? value + checksumITF14(value) : value
...
export function normalizeBarcodeOptions(options: BarcodeOptions = {}): BarcodeRenderOptions {
...
export function barcode(value: string, options: BarcodeOptions = {}) {
  ...
  const encoder = createEncoder(normalizedOptions.format, value, normalizedOptions)

  if (!encoder.valid()) {
    throw new Error(`Invalid barcode input for format ${normalizedOptions.format}: ${value}`)
  }

  const encodings = encoder.encode()
  const normalizedEncodings = Array.isArray(encodings) ? encodings : [encodings]

  return normalizedEncodings.map((encoding) => normalizeRawEncoding(encoding, normalizedOptions))
}
...
function getBarcodePadding(textWidth: number, barcodeWidth: number, options: BarcodeRenderOptions) {
...
export function calculateBarcodeLayout(
  ...
  const segments = encodings.map<BarcodeLayoutSegment>((segment) => {
    const textWidth =
      segment.options.displayValue && segment.text ? measureText(segment.text, segment.options) : 0
    ...
    return {
      ...
      textWidth,
      barcodeWidth,
      barcodePadding: getBarcodePadding(textWidth, barcodeWidth, segment.options),
    }
  })

  const totalContentWidth = segments.reduce((sum, segment) => sum + segment.width, 0)
  const baseOptions = segments[0]?.options ?? normalizeBarcodeOptions()
  const height = segments.reduce((maxHeight, segment) => Math.max(maxHeight, segment.height), 0)
