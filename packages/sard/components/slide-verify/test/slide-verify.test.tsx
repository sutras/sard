...
import type { SlideVerifyProps } from '../common'

describe('SlideVerify', () => {
  test('Basic', async () => {
    const mockVerify = vi.fn<NonNullable<SlideVerifyProps['verify']>>()

    const wrapper = mount(
      <SlideVerify text="请按住滑块拖动" successText="验证通过" verify={mockVerify}></SlideVerify>,
    )

    expect(wrapper.find('.s-slide-verify__track-text').text()).toEqual('请按住滑块拖动')
  ...
  test('ShowTarget', async () => {
    ...
    expect(wrapper.find('.s-slide-verify__target').exists()).toBeTruthy()
  })
})
