  ...
  test('Basic', async () => {
    const wrapper = mount(
      <Sidebar modelValue={'2'}>
        <SidebarItem value="1" label="标签名称" />
        <SidebarItem value="2" disabled label="标签名称" />
        <SidebarItem value="3" label="标签名称" />
      </Sidebar>,
    )

    // default active
    expect(wrapper.find('.s-sidebar-item:nth-child(2)').classes()).includes('is-active')
    // disabled
    ...
    expect(wrapper.find('.s-sidebar-item:nth-child(3)').classes()).includes('is-active')

    // set props
    await wrapper.setProps({
      modelValue: '1',
    })
    expect(wrapper.find('.s-sidebar-item:nth-child(1)').classes()).includes('is-active')
