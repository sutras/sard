<template>
  <Picker :columns="columns" :model-value="pickerValue" @change="onChange" />
</template>

<script setup lang="ts">
import { computed, ref, watch } from 'vue'
import { formatDate, lunarToSolar, solarToLunar, toDate } from '../../utils'
...
const props = withDefaults(defineProps<DatetimePickerProps>(), defaultDatetimePickerProps)
...
const createColumnData = (types: DatetimeLetter[], currentDate: Date) => {
  ...
  return types.map((letter) => (columnsMap[letter] = getColumnDataByType(letter as DatetimeLetter)))
}

const getChangedLetter = (currentDate: Date) => {
  const min = getBoundaryValue(props.calendar, false, minDate.value, currentDate)
  const max = getBoundaryValue(props.calendar, true, maxDate.value, currentDate)

  return letterArray.filter((_, i) => min[i] !== minValues[i] || max[i] !== maxValues[i])
}
...
const getDateByPickerValue = (value: number[]) => {
  ...
  correctDate(props.calendar, currEvery as DateEvery, minDate.value, maxDate.value)
