...
export const defaultDatetimePickerProps: DefaultProps<DatetimePickerProps> = {
  type: 'yMd',
  calendar: 'solar',
}

export interface DatetimePickerSlots extends PickerSlots<number> {}
...
function getSolarBoundaryValue(isMax: boolean, endDate: Date, currentDate: Date) {
...
function getLunarBoundaryValue(isMax: boolean, endDate: Date, currentDate: Date) {
...
export function correctSolarDate(date: DateEvery, minDate: Date, maxDate: Date) {
  let minAside = true
  let maxAside = true
...
export function correctLunarDate(date: DateEvery, minDate: Date, maxDate: Date) {
  ...
  letterArray.slice(1).forEach((letter, index) => {
    ...
    if (letter === 'd') {
      range[1].value =
        date[1] < 0 ? getLunarLeapMonthDays(date[0]) : getLunarMonthDays(date[0], date[1])
    }
...
export const normalizeRangeValue = (
  ...
  const endValue = end || getInitialValue(toDate(startValue, valueFormat), maxDate)

  return [
    valueFormat && isDate(startValue) ? formatDate(startValue, valueFormat) : startValue,
    valueFormat && isDate(endValue) ? formatDate(endValue, valueFormat) : endValue,
  ]
}
