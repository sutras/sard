...
const createChildren2 = () => {
  return [
    <TabbarItem value={0} label="首页" />,
    <TabbarItem value={1} label="购物车" />,
    <TabbarItem value={2}>
...
const createChildren3 = () => {
  return [
    ...
    <TabbarItem value={2} label="消息" />,
    <TabbarItem value={3} label="我的" />,
  ]
}

describe('Tabbar', () => {
  test('basic', async () => {
    const wrapper = mount(<Tabbar modelValue={1}>{createChildren()}</Tabbar>)

    expect(wrapper.findAll('.s-tabbar__item')[1].classes()).toContain('s-tabbar__item--active')
    expect(wrapper.find('.s-tabbar__item:nth-child(3) .s-tabbar__label').text()).toBe('消息')
    expect(wrapper.find('.s-tabbar__item:nth-child(3) .message-icon').exists()).toBe(true)
  })

  test('color', async () => {
    const wrapper = mount(
      <Tabbar modelValue={1} activeColor="red">
        {createChildren()}
      </Tabbar>,
    )

    expect(wrapper.find('.s-tabbar__item--active').attributes('style')).toContain('color: red;')
  })
  ...
  test('bulge', async () => {
    const wrapper = mount(<Tabbar modelValue={1}>{createChildren3()}</Tabbar>)
  ...
  test('safeAreaInsetBottom', async () => {
    const wrapper = mount(
      <Tabbar modelValue={1} safeAreaInsetBottom>
        {createChildren3()}
      </Tabbar>,
    )
