...
| children      | 可以为一个接受状态的函数，或者为 React 元素                                                                                           | ReactElement\| ((status: Status, props: { [p: string]: any }) => ReactElement \| null) | -      |
...
| type | 用来使用内置的动画效果；`name` 属性优先级较高                                                                                          | 'fade' \| 'slide-top' \| 'slide-right' \| 'slide-bottom' \| 'slide-left' \| 'zoom' \| 'collapse' | 'fade' |
