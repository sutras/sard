...
export const PickerViewColumn = (props: PickerViewColumnProps) => {
  ...
  const handleTouchMove = useEvent((event: PanEvent) => {
    ...
    if (moveCoordQueue.current.length > maxLastMoveCoordCount) {
      moveCoordQueue.current = moveCoordQueue.current.slice(
        -maxLastMoveCoordCount,
      )
    }
  ...
  return (
    <PanGestureDetector handlers={handlers}>
      <View className={bem.e('column')}>
        <View className={classNames(bem.e('mask'))} />
        <View
