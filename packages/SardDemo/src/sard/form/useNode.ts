...
export function isMapNodeValue(map?: object) {
  return isPlainObject(map) && map[mapSymbol]
}
...
export function useNode(options: {
  ...
  const getNamePath = useEvent(() => {
    return name ? getNamePathByCurrentName(name, parentNode) : []
  })
  ...
  const { getFormInitialValues } = (formStore as FormStore).getInternalHooks(
    HOOK_KEY,
  )
        ...
        switch (parentNodetype) {
          ...
          case 'form':
            currentValue = markMapNodeValue({
              [currentName as NodeName]: currentValue,
            })
            break
          case 'list': {
            const list = markListNodeValue([])
            list[currentName as NodeName] = currentValue
            currentValue = list
          }
        }
  ...
  const getDescendantNode = useEvent((name: NamePath) => {
    ...
    let currentNode: FormNode | null = null
