...
export function useValidator(options: {
  ...
  const { validator } = (formStore as FormStore).getInternalHooks(HOOK_KEY)
  ...
  const ruleRequired = useMemo(() => {
    return !!(
      rules &&
      rules.some((rule) => getRuleConfig(rule, formStore as FormStore).required)
    )
  }, [rules])
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (triggerName) {
        filteredRules = rules.filter(Boolean).filter((rule) => {
          const ruleConfig = getRuleConfig(rule, formStore as FormStore)
