...
export const Tabs: TabsFC = (props) => {
  ...
  const [notSetLineStyleYet, setNotSetLineStyleYet] = useState(true)
  ...
  const setTabStyle = async (key: string | number, onlyLine?: boolean) => {
    ...
    // line
    if (showLine) {
      const lineLeft = tabRect.left - wrapperLeft + tabRect.width / 2
      setLineLeft(lineLeft)
      setNotSetLineStyleYet(false)
      if (onlyLine) {
        return
      }
    }
  ...
  const animateLineTranslateX = (to: number) => {
    Animated.timing(lineAnim, {
      toValue: to,
      duration: notSetLineStyleYet ? 0 : duration,
              ...
              {showLine && (
                <Animated.View
                  ...
                  style={
                    {
                      ...lineStyle,
                      opacity: notSetLineStyleYet ? 0 : 1,
                      ...(isRN
                        ? {
                            transform: [
                              { translateX: lineAnim },
                        ...
                        : {
                            transition: `transform ${
                              notSetLineStyleYet ? 0 : duration
                            }ms ease-out 0s`,
                            transform: `translate3d(${lineLeft}px,0,0) translateX(-20px)`,
                          }),
                    } as any
                  }
                >
