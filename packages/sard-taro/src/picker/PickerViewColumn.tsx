...
import { useBem, useBrush, useEvent } from '../use'
...
export const PickerViewColumn = (props: PickerViewColumnProps) => {
  ...
  const currentIndex = useRef(minmax(value, 0, itemCount - 1))
  ...
  const handleTouchMove = useEvent((event: ITouchEvent) => {
    ...
    setTranslate(
      getCurrentDampedVaule(downTranslate.current + brush.deltaY, true),
    )
  })
