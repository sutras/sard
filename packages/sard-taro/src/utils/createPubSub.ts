export interface PubSub {
  on(type: string, handler: (...args: unknown[]) => unknown): void
  off(type?: string, handler?: (...args: unknown[]) => unknown): void
  emit(type: string, ...payload): void
  once(type: string, handler: (...args: unknown[]) => unknown): void
}
...
export function createPubSub(): PubSub {
  ...
  const emit: PubSub['emit'] = (type, ...payload) => {
    ...
    handlers.forEach((handler: (...args: unknown[]) => unknown) => {
      handler(...payload)
    })
  }
